...
void update()
{
	...
	if (S_OK != URLDownloadToFile(NULL, _T("https://www.mcjiaozi.icu/update/bamboo/version.txt"), _T("\\bambooDownload\\version.txt"), 0, 0)) {
	...
	if (ch > 53) {
		...
		if (updateaa == IDOK) {
			...
			if (S_OK != URLDownloadToFile(NULL, _T("https://www.mcjiaozi.icu/download/software/bamboo/bamboo_installer.exe"), _T("\\bambooDownload\\bamboo_installer.exe"), 0, 0)) {
				MessageBox(NULL, _T("下载更新失败！\n请前往在线更新网站！"), _T("错误"), MB_OK | MB_ICONERROR);
				return;
			}
			ShellExecute(NULL, NULL, _T("cmd"), _T("/c start \\bambooDownload\\bamboo_installer.exe"), NULL, SW_HIDE);
			return;
		}
		else if (updateaa == 0) {
